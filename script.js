...
// variables related to speech synthesis
...
img.addEventListener('load', () => {
  ...
  // need to enable submit button since any existing text is cleared
  if (img.src == "") {
...
document.getElementById("generate-meme").addEventListener('submit', function(e) {
  ...
  // align and draw text
  let spacing = canvas.height * 0.1; // vertical padding for text
  context.textAlign = "center";
  context.font = "bold 40px arial";
  // outline 
  context.strokeStyle = 'black';
  context.lineWidth = 8;
  context.strokeText(toptext, canvas.width/2, spacing);
  context.strokeText(bottext, canvas.width/2, canvas.height-(spacing/2));
  // fill
  context.fillStyle = 'white';
  context.fillText(toptext, canvas.width/2, spacing);
  context.fillText(bottext, canvas.width/2, canvas.height-(spacing/2));
...
document.querySelector("[value='none']").remove();
voiceSelect.disabled = false;

// Speech synthesis function
function speak(txt) {
  let speech = new SpeechSynthesisUtterance(txt);
  speech.volume = document.querySelector("[type='range']").value/100; // set volume based on value of the slider
